...
def _ambient_generator(data_processor, mode, features_length, chunk_size=1024):
    """Generator that yields chunks of (spectrogram, label) for the ambient set.
    Chunking significantly reduces the overhead of yielding samples one by one."""
    chunk_data = []
    chunk_labels = []
    for provider in data_processor.feature_providers:
        gen = provider.get_feature_generator(mode, features_length, "split")
        for spectrogram in gen:
            chunk_data.append(spectrogram)
            chunk_labels.append(np.array([provider.label], dtype=np.float32))

            if len(chunk_data) >= chunk_size:
                yield np.array(chunk_data, dtype=np.float32), np.array(chunk_labels, dtype=np.float32)
                chunk_data = []
                chunk_labels = []
...
def validate_nonstreaming(config, data_processor, model, test_set, skip_ambient=False):
    ...
    if not skip_ambient and data_processor.get_mode_size("validation_ambient") > 0:
        # Build a tf.data.Dataset from a generator to avoid loading all ~190K
        # ambient samples into a single numpy array (which causes OOM).
        spectrogram_shape = testing_fingerprints.shape[1:]  # e.g. (T, F)
        ambient_eval_batch_size = config.get("ambient_eval_batch_size", 256)

        ambient_mode = test_set + "_ambient"
        features_length = config["spectrogram_length"]

        def _gen():
            yield from _ambient_generator(data_processor, ambient_mode, features_length, chunk_size=1024)

        dataset = tf.data.Dataset.from_generator(
            _gen,
            output_signature=(
                tf.TensorSpec(shape=(None, *spectrogram_shape), dtype=tf.float32),
                tf.TensorSpec(shape=(None, 1), dtype=tf.float32),
            ),
        ).unbatch().batch(ambient_eval_batch_size).prefetch(tf.data.AUTOTUNE)
        ...
        # both test sets
        all_true_positives = np.array(ambient_predictions["tp"])
        ambient_false_positives = np.array(ambient_predictions["fp"]) - test_set_fp
        all_false_negatives = np.array(ambient_predictions["fn"])
...
def train(model, config, data_processor):
    ...
    for training_step in range(1, training_steps_max + 1):
        ...
        # Two-tier validation:
        #   full_eval_interval  (default = 5 * eval_step_interval): light + ambient FAPH
        #   eval_step_interval : light only (fast, no ambient)
        full_eval_interval = config.get(
            "full_eval_interval",
            config["eval_step_interval"] * 5,
        )
        is_light_eval_step = (training_step % config["eval_step_interval"]) == 0
        is_full_eval_step = (training_step % full_eval_interval) == 0 or is_last_step

        if is_light_eval_step or is_last_step:
            logging.info(
            ...
            # ── LIGHT VALIDATION (fast, no ambient) ──────────────────────────
            light_metrics = validate_nonstreaming(
                config, data_processor, model, "validation", skip_ambient=True
            )
            model.reset_metrics()
            logging.info(
                "Step %d [light-val]: accuracy=%.2f%%, recall=%.2f%%, precision=%.2f%%, loss=%.5f, auc=%.5f",
                training_step,
                light_metrics["accuracy"] * 100,
                light_metrics["recall"] * 100,
                light_metrics["precision"] * 100,
                light_metrics["loss"],
                light_metrics["auc"],
            )

            with validation_writer.as_default():
                tf.summary.scalar("loss", light_metrics["loss"], step=training_step)
                tf.summary.scalar("accuracy", light_metrics["accuracy"], step=training_step)
                tf.summary.scalar("recall", light_metrics["recall"], step=training_step)
                tf.summary.scalar("precision", light_metrics["precision"], step=training_step)
                tf.summary.scalar("auc", light_metrics["auc"], step=training_step)
                validation_writer.flush()

        if is_full_eval_step:
            # ── FULL VALIDATION (with ambient FAPH) ──────────────────────────
            logging.info("Step %d [full-val]: running ambient validation...", training_step)
            nonstreaming_metrics = validate_nonstreaming(
                config, data_processor, model, "validation", skip_ambient=False
            )
            model.reset_metrics()
            logging.info(
                "Step %d [full-val]: recall_at_no_faph=%.3f (cutoff=%.2f), accuracy=%.2f%%, recall=%.2f%%, precision=%.2f%%, ambient_fp=%d, faph=%.5f, loss=%.5f, auc=%.5f, avg_viable_recall=%.9f",
                *(
                    training_step,
            ...
            with validation_writer.as_default():
                ...
                tf.summary.scalar("faph", nonstreaming_metrics["ambient_false_positives_per_hour"], step=training_step)
                validation_writer.flush()
